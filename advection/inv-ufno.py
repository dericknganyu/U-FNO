...
parser.add_argument('--res', default=200-1, type = int, help='resolution')
...
print(padding)
model = UFNO1d(modes, width, padding).cuda()
...
for ep in range(epochs):
    ...
    for x, y in train_loader:
        ...
        optimizer.zero_grad()
        out = model(x).reshape(batch_size, res)
...
ff = ff.reshape(1,res,1).cuda()#torch.cat([ff.reshape(1,res,1), grid.repeat(1,1,1,1)], dim=3).cuda()

U_FNO = y_normalizer.decode(model(ff).reshape(1,res)).detach().cpu().numpy()
U_FNO = U_FNO[0] 
...
colourMap = plt.cm.magma # plt.cm.viridis #parula() #plt.cm.jet #plt.cm.coolwarm
...
plt.ylabel('y')#, fontsize=16, labelpad=15)
plt.title("UFNO")
plt.plot(U_FNO)#, cmap=colourMap, extent=[params["xmin"], params["xmax"], params["ymin"], params["ymax"]], origin='lower', aspect = 'auto')#, vmin=0, vmax=1, )
#plt.colorbar()#format=OOMFormatter(-5))
...
plt.plot(np.abs(U_FDM - U_FNO))#, cmap=colourMap, extent=[params["xmin"], params["xmax"], params["ymin"], params["ymax"]], origin='lower', aspect = 'auto', norm=matplotlib.colors.LogNorm())#, vmin=0, vmax=1, )
#plt.colorbar()#format=OOMFormatter(-5))
plt.yscale('log')
